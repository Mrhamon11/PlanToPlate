...
| 04 | [Units & Ingredients](04-Units-And-Ingredients/design.md) | IN PROGRESS | — | Subtasks 04.1–04.5 done (Unit + Dimension, Tag, conversion service, Ingredient model, seed data + `seed_catalog`); pipeline converged on iteration 2 (tester PASS, reviewer APPROVE) after iteration 1's 2 blocking findings — COUNT-unit false conversions (fixed via `Unit.count_family`, see D34) and a Decimal-vs-float guard test that passed against a float impl. 407 passed, ruff clean, no pending migrations. **04.6–04.13 remain** (serializers, API viewsets, protected-delete, UI, unit picker, quick-add). Resume with `/p2p-task 04`. Awaiting owner approval of this partial run before commit. |
