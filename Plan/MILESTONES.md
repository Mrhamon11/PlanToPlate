...
| 06 | [Dishes & RecipeBooks](06-Dishes-And-RecipeBooks/design.md) | COMPLETE | 2026-09-03 | The `meals/` app: `Dish` / `DishComponent` (`servings` scale factor, `PROTECT` on recipe) and `RecipeBook` / `RecipeBookEntry` (free-text `section`, `default_ordering`, `CASCADE` so a deleted recipe leaves its books quietly). `Dish.flatten()` scales each component by `servings` and aggregates across the dish — **this is the shopping list's entry point for tasks 07/08**. Per-user stats now sit on the abstract `core.UserObjectStats` base shared by `RecipeStats` and `DishStats` (D38). REST viewsets + HTMX list / detail / form screens for both, "Add to book ▾" on the recipe page, large-book copy warning, touch up/down reordering. Still open: a share **recipient** cannot remove a shared object from their own view, which also makes a copy show up alongside the original (11.19); the dish form rejects the same recipe twice with a misleading message while the API allows it (11.20); the book-detail fragment endpoints are N+1 on book size (11.21). |
| 07 | [Lists & Shopping](07-Lists-And-Shopping/design.md) | COMPLETE | 2026-09-06 | The `lists/` app: `List` / `ListItem` (four nullable content FKs, has-content check, `SET_NULL` + `pre_delete` tombstone receiver — D41), and `lists/services.py` (race-safe default shopping list, add recipe/dish to list, `populate_shopping_list` = **the task-08 contract** for meal-plan-driven aggregation with replace-only-this-plan's-generated-items, merge/clear/bulk-check, `update_item` = the one quantity/unit validation choke point — D45). Serializers + REST API; phone-first HTMX UI (kind-grouped index, aisle-grouped shopping detail with OOB one-tap check + live action bar, inline quantity/unit edit, bulk check/clear, generic list detail, "Add to list ▾" on recipe/dish pages). A shared list is read-only for the recipient (D40; editable sharing → task 13, D46). Still open for task 08: flesh out the `planner.MealPlan` stub (D44); the real planner-driven regeneration flow (07 ships only the manual clear-generated counterpart); provenance-label display (D43); findings 3 & 7 and the two-FK `pre_delete` blind spot (D41). |
| 08 | [Meal Planner](08-Meal-Planner/design.md) | COMPLETE | 2026-09-07 | The `planner/` app: `MealPlanProfile` (the eight gears, capped — `exclude_staples` is an acknowledged shopping-list 9th knob), `MealPlan` / `MealPlanEntry` with `seed` + `profile_snapshot`. Seeded generator (`random.Random(seed)`, weighted greedy + bounded backtracking, byte-identical under a fixed seed, degrades honestly with per-slot reasons), candidate pool built exclusively from `Dish.objects.visible_to(user)`, transient dish composition (protein+carb+vegetable) persisted only on save and never repeated within a plan. Regeneration respects `is_locked` and locked tag budgets; persist is stateless (re-runs the generator from `{profile, start_date, seed, days}`). Shopping-list integration is pure orchestration over task 07's `populate_shopping_list` — no flatten/aggregate logic in `planner/`. REST API + phone-first HTMX UI (profile editor, generate + week grid, per-slot lock/reroll/swap, shopping preview with tri-state staples, empty-state guidance, rename/delete/share). Sharing a plan **cascades** read-grants to its scheduled dishes and their recipe graphs (D44 amended, D47); `profile_snapshot` is owner-only on read (D48); `MealPlanViewSet.copy` is 405. Still open: persisted per-slot unfilled `reason` and several result-quality/perf nicks in `Plan/BACKLOG.md`. |
...
| 12 | [Home Dashboard](12-Home-Dashboard/design.md) | COMPLETE | 2026-09-08 | Replaces task 02's placeholder card grid with a real dashboard. `core.services.dashboard.build_dashboard(user)` is the single read model — this week's plan, active shopping list, recently viewed, favourites, shared-with-you, public-from-others, a dish suggestion, section counts — consumed unchanged by the HTMX home page and `GET /api/dashboard/`. New `core.RecentView` (generic-FK per-user telemetry, one bumped row per `(user, object)`, capped, **not** an `OwnedModel`); recipe/dish/meal detail views record a view. Every panel query goes through `.visible_to(user)` and stored-state rows are re-resolved live. Panels with nothing to say are absent; the five section links are the floor. See D51, D52. Still open: `_this_week` considers only the most-recently-*started* owned plan (misses an older overlapping plan); polish + test-gap items folded into `BACKLOG.md` "Home dashboard — follow-ups" / the N4 pass. |
| 13 | [Collaborative Sharing](13-Collaborative-Sharing/design.md) | NOT STARTED | — | Editable shares: a share can grant **edit** as well as the current view-only (recipient reads / copies-to-modify, original untouched). Applies to recipes, dishes, books, and lists. **Spec required first** — `design.md` / `test-plan.md` written before any code. Open questions: concurrent edits (two people ticking one shopping list), whether edit rights cascade to child objects, interaction with `copied_from`. Builds on task 03. |
...
**Dependency order:** 00 → 01 → 02 → 03 → 04 → 05 → {06, 07} → 08 → 12 → 09 → 10 → 13.
...
subtask is done. Task 13 (editable sharing) builds on task 03 and needs its own spec written
first; sequenced after 10 but not blocked by it.

Task 12 sits where it does because its two headline panels need Lists (07) and Planner (08) to
exist; building it earlier would ship a dashboard of leftovers and guarantee a second pass.
Until then the home page's section cards stay correct but redundant — 07.13a and 08.14a light
up the last two.
