...
| 02 | [UI Shell](02-UI-Shell/design.md) | COMPLETE | 2026-08-24 | All 12 subtasks done; pipeline converged (tester PASS, correctness reviewer APPROVE, security reviewer APPROVE) after iteration 3's 5 blocking correctness findings and one authorised 4th pass, then the project owner's own real-browser manual QA on the running dev server found 3 further real defects (pass 5: H1-H3) and 2 small follow-ups (pass 6: F1-F2), all fixed and re-verified before the owner approved closing the task. `base.html`, hand-written mobile-first CSS (`reset.css`/`app.css`/`components.css`/`nav.css`) with light/dark design tokens, a responsive top-bar/bottom-tab nav (native `<details>` dropdowns, zero JS required, a "Home" tab on the bottom bar), the five shared partials, htmx 2.0.10 and Alpine 3.16.3 vendored into `static/js/` (see `static/js/VENDOR.md`), `core.middleware.HtmxMiddleware` (request-flag + `HX-Boosted` flag + generic redirect→`HX-Redirect` rewrite, mutating the response in place so `Set-Cookie`/`X-Frame-Options` survive), `core.mixins.HtmxTemplateMixin`/`MessageMixin`/`OwnedObjectMixin`, the task 01 auth templates restyled onto `base.html`, a real `core:home` dashboard shell, and styled 403/404/429/500 error pages — 429 fills the D25 gap with a template that keeps `Retry-After` and states no account is locked. **156 passed**, ruff clean, `manage.py check` and `check --deploy` clean. Three deliberate deviations from the literal task/file list, all functional not architectural: (1) `LOGIN_REDIRECT_URL` now points at `core:home` instead of `accounts:profile` — base.py's own comment flagged this as task 02's job, and `test_login_success_redirects_to_home` (already named for it) was updated to match; (2) the home route was added to the existing `core/urls.py` (`app_name = "core"`) rather than `config/urls.py`, since `core.urls` is already included there — same net effect, no new top-level wiring; (3) the 429 template render lives in `accounts/throttling.py`'s shared `check_login_throttle` (the one place both the HTML login view and `/admin/login/` already funnel through) rather than duplicated into `accounts/views.py`. `STATICFILES_DIRS`'s `.exists()` guard was also simplified to unconditional now that `static/` is a permanent, committed directory. See D30 for a local-dev static-serving gotcha found during manual QA. **The formal 7-item × 3-width manual responsive checklist was not performed** — the owner instead tested live in a real browser, which is how H1-H3/F1-F2 were found; closed out on explicit owner approval despite the gap (see `02-UI-Shell/test-plan.md`'s Definition of Done). |
| 03 | [Ownership & Sharing](03-Ownership-And-Sharing/design.md) | COMPLETE | 2026-08-26 | All 12 subtasks done and approved by the project owner, who will commit, push, and merge the branch himself. No live UI to click yet — this task builds infrastructure (permissions, mixins, dummy-model tests) that tasks 04–08 inherit, tested only against `core.tests.models.DummyOwned`/`DummyNode`/`DummyContainer` fixtures, not a real screen. This run (03.8a, 03.9, 03.10, 03.11) took 3 review iterations to converge, on top of an internal tester-FAIL/fix cycle before the first review: iteration 1 (two parallel reviewers, security+correctness) found 7 blocking findings — `core/viewsets.py`'s `get_permissions()` silently discarding subclass/action-level `permission_classes`; `test_conventions.py`'s two regression-guard tests (`test_all_owned_viewsets_use_visible_to`, `test_all_owned_models_declare_hooks`) too weak to catch real future violations (wrong module-name assumption, no reverse-relation/join-model detection); `_share_modal.html` leaking the owner-only shares audience list to any visible-to viewer; `OwnedObjectMixin` having no HTML-side equivalent of `OwnedSerializer`'s read-only-field guard; and design.md's copy-provenance UI line being silently unimplemented. Iteration 2's fix for the permission-composition finding introduced a *new* flagship IDOR by having an explicit `permission_classes` override *replace* rather than compose with the mixin's ownership baseline (a read-only share holder could DELETE and read `/shares/`), and its join-model hooks-guard fix false-positived on leaf models (e.g. would have flagged a future `Ingredient` once `RecipeComponent` exists) — caught by iteration 3's single combined reviewer, per the project owner's standing instruction to default to one reviewer unless a task genuinely needs split focus. Iteration 3 fixed both properly (permission classes now compose additively across all three layers; leaf models get an explicit `OwnedModel.contains_owned_children` opt-out consulted before the heuristic) plus a null-owner render bug in the new `_copied_from.html` partial (copying a system/seeded object crashed the provenance line), and converged with APPROVE. 360 passed (up from 226 at the end of 03.1–03.4), ruff clean, `manage.py check` clean, no missing migrations. Non-blocking gap flagged for task 05/06: `_copied_from.html` follows the original's *live* title/owner via FK rather than denormalizing at copy time, so a copy's provenance line can later disclose a renamed or now-private original's current name to anyone who can see the copy — not fixed here since no screen renders the partial yet and the real fix is a design call for whichever task first surfaces it. |
| 04 | [Units & Ingredients](04-Units-And-Ingredients/design.md) | IN PROGRESS | — | Subtasks 04.1–04.5 done (Unit + Dimension, Tag, conversion service, Ingredient model, seed data + `seed_catalog`); pipeline converged on iteration 2 (tester PASS, reviewer APPROVE) after iteration 1's 2 blocking findings — COUNT-unit false conversions (fixed via `Unit.count_family`, see D34) and a Decimal-vs-float guard test that passed against a float impl. **04.6–04.12 implemented (p2p iteration 1, last completed subtask 04.12), awaiting tester + reviewer.** Serializers (`catalog/serializers.py`), API viewsets + filters (`catalog/api.py`/`filters.py`/`api_urls.py`, staff-gated Unit/Tag writes, `POST /api/units/convert/`, `IngredientViewSet` via `OwnedViewSetMixin`), protected-delete → 409 (`core/exceptions.py`), the ingredient list/detail/form/quick-add HTMX screens (`catalog/views.py`, `catalog/urls.py`, `templates/catalog/*`), the shared `_partials/_unit_select.html`, and share/copy wired to task 03's partials via HTML intermediary views. 472 passed, ruff + `check` + `makemigrations --check` clean. Migration `catalog/0002` adds `Unit.abbrev` unique (carried finding #7). **04.8's real end-to-end `test_delete_in_use_returns_409` is DEFERRED to task 05** — nothing PROTECTs `Ingredient` until `RecipeComponent`; a synthetic `ProtectedError` unit test + a monkeypatched viewset test stand in. **Finding #11 resolved by dropping `visibility=PUBLIC` from the ingredient seed** (system rows are readable via `is_system=True`, and PUBLIC made `?public=true` return all ~176 built-ins) — pending owner confirmation. **04.13 does not flip status to AWAITING APPROVAL** — the orchestrator does that after human approval. `design.md` "Same dimension: always works" line is stale re: D34 (finding #19) — not edited, flagged for owner. Resume with `/p2p-task 04`. |
...
| 11 | [Task Bug Fixes](11-TaskBugFixes/tasks.md) | NOT STARTED | — | Standing catch-all for reviewer findings deliberately deferred rather than fixed in-task. Established from task 03's review (NB2, NB6, NB7, NB8); no `design.md`/`test-plan.md` yet by design — written per-subtask when picked up. |
...
Task 11 is not part of this chain — it's a standing catch-all with no fixed slot, picked up
whenever convenient once the task that surfaced a given subtask is already done.
...
| D30 | `.env` and `.env.example` both default `DEBUG=false`, which silently breaks static-file serving (CSS/JS all 404) for anyone running `manage.py runserver` against a fresh checkout — found live during task 02's manual QA | Two independent things gate on `DEBUG` here: Django's `runserver` only auto-serves `STATICFILES_DIRS` (where `static/css/*.css` actually live) when `DEBUG=True`, and WhiteNoise's middleware serves from `STATIC_ROOT` (`staticfiles/`) instead, which is only ever populated by `collectstatic` — never run in local dev. With `DEBUG=false`, neither path serves anything, and the failure is quiet: the page returns 200, the `<link>` tags are correct, only the browser shows unstyled HTML, plus a startup `UserWarning: No directory at: .../staticfiles/` that reads like unrelated noise. Fixed for this session by setting the local `.env`'s `DEBUG=true` (a gitignored file, no code change). Not yet fixed: `.env.example` — the template a fresh clone copies — still defaults to `false`, so this will resurface for the next person who runs `cp .env.example .env` and `runserver` without reading this entry. Recorded rather than changed, since flipping `.env.example`'s default is a judgment call (arguably `DEBUG=false` is the safer copy-paste default for a security-sensitive project) that the next session or the project owner should make deliberately, not inherit as a silent side effect of a UI-shell task. |
| D31 | `_cascade_grant_public`'s widening of the actor's own dependencies to `PUBLIC` is deliberate and stays irreversible on revert, by the same asymmetry as "unsharing does not cascade" — confirmed with the project owner, closing the review's open question rather than leaving it as a defect | There is no per-model mechanism to grant "everyone" read access to a child other than setting that child's own `visibility`, so publishing a container has to widen owned children the same way sharing it cascades `shared_with` to them. Dropping the container back to `PRIVATE` or `SHARED` does not un-publish those children, exactly as revoking a share does not revoke a child's grant — recorded in `design.md`'s Edge cases, and the API's `share` response now returns `cascaded_to` so a caller can see which of its own objects a `PUBLIC` widening is about to affect. |
| D32 | `OwnedViewSetMixin.get_permissions()` composes three permission layers additively — the live `api_settings.DEFAULT_PERMISSION_CLASSES`, the mixin's own action-keyed ownership baseline (`IsOwner`/`CanCopy`/`IsOwnerOrReadOnly`), and any subclass- or `@action`-level `permission_classes` declaration — and a declared override can only **add** classes, never drop the baseline | Found during task 03's final review round: an earlier fix let an explicit `permission_classes` declaration *replace* the mixin's baseline instead of composing with it, reopening the flagship IDOR — a viewset declaring `permission_classes = [IsAuthenticated]` let a read-only share holder `DELETE` the object and read its owner-only `/shares/` audience list. Any task 04+ viewset that narrows or extends permissions for one action must layer on top of `OwnedViewSetMixin`'s baseline, never assign `permission_classes` as a full replacement. |
| D33 | `OwnedModel` carries an explicit `contains_owned_children: bool | None = None` opt-out, consulted by `core/tests/test_conventions.py`'s hooks-guard test before its relation-walk heuristic | The heuristic that flags a model missing `share_dependencies()`/`copy_children()` cannot tell "container reached via a join model" from "leaf reached via the same join model" — both sides of a two-parent join table (the real `RecipeComponent.recipe`/`.ingredient` shape task 05 will add) look identical to a relation walk. A leaf model (e.g. `Ingredient`) declares `contains_owned_children = False` to opt out deliberately and greppably, rather than silencing the guard with a no-op hook implementation that looks identical to a genuine one. See `core/README.md` for the full contract. |
| D34 | **COUNT ↔ COUNT conversion contract.** `Unit` carries a `count_family: str` discriminator (blank for MASS/VOLUME). Counted units split into two classes: the **generic** family (`each` = 1, `half dozen` = 6, `dozen` = 12) whose members interconvert on those real ratios via `to_base_factor`; and every packaging/piece unit (can, slice, clove, pinch, package, bunch, head, stalk, sprig, stick, leaf, piece) which is its **own singleton family** and converts *only to itself*. `catalog.services.units.convert` raises `IncompatibleUnits` — with a reason naming both units — for any COUNT↔COUNT pair not sharing a non-empty family. Cross-dimension COUNT↔MASS/VOLUME remains categorically refused, unchanged. | Found in task 04's iteration-1 review. All thirteen discrete counted units were seeded with `to_base_factor = 1`, so the same-dimension branch treated them as freely interchangeable: `convert(2, clove, can)` returned `2`, `convert(3, pinch, each)` returned `3`. That is the exact "confidently incorrect shopping list" the design forbids for MASS↔VOLUME, arriving through the one path left unguarded — task 05's shopping-list aggregation converts component lines to a common unit before merging, so "1 can Diced Tomatoes" + "2 slices Tomato" would have aggregated to "3 each". `each`/`dozen`/`half dozen` genuinely do have a fixed ratio and must keep interconverting (that half was already correct and tested); the fix is to stop pretending a clove and a can do. Task 05's `flatten`/aggregation must treat an `IncompatibleUnits` between two counted units the same way it treats a missing density — keep the lines separate rather than inventing a number. |
