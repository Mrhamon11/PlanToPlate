...
| 03 | [Ownership & Sharing](03-Ownership-And-Sharing/design.md) | IN PROGRESS | — | Subtasks 03.1–03.4 done and reviewer-approved (last completed: 03.4, dependency-graph traversal helper); 03.5–03.12 (sharing/copy services, serializers, viewset, HTML mixins, UI, docs, final sign-off) remain. Resume with `/p2p-task 03`. Built against a throwaway `core.tests.models.DummyOwned`/`DummyNode` fixture app (test settings only) since no domain model exists yet. Pipeline took 3 iterations to converge: iteration 1 found 5 blocking findings (exponential graph walk, `IsOwnerOrReadOnly` safe-method visibility bypass, anonymous `None==None` write-access bug, silently-droppable owner-XOR-system constraint on a subclass with its own `Meta`, `walk_dependencies` hard-wired to one edge function); iteration 2 fixed all 5 and added a `django.core.checks` system check (`core/apps.py`) as a backstop for the constraint issue, but its own reviewer found 3 more findings, all in test quality (a tautological assertion, an assertion that didn't actually check registry wiring, an untested memoization edge case) rather than production bugs; iteration 3 fixed those and converged with a single combined reviewer (security+correctness) approving. 226 passed, ruff clean, `manage.py check` clean. One reviewer-identified non-blocking gap flagged for before task 04: the `core.apps` system check reads `model.objects`, so a future `OwnedModel` subclass that declares a second manager (making `_default_manager` something other than `objects`) would pass the check but fail closed at runtime with an `AttributeError` in `IsOwnerOrReadOnly` rather than being caught at `manage.py check` time — an additive `core.E003` assertion on `_default_manager` was scoped but not implemented pending approval. |
